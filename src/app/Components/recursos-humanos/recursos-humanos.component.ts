...
import { getAllFiles, getFile, getPossibleFiles } from 'src/app/Operations/query';
...
export class RecursosHumanosComponent implements OnInit {
  ...
  getFiles(): void {
    this.query.executeQuery(this.query.getOptions(getPossibleFiles, "network-only", { departamento: "recursoshumanos" },
  ...
  getAllFiles() {
    this.query.executeQuery(
      this.query.getOptions(getPossibleFiles, "network-only", { departamento: "recursoshumanos" }, { headers: this.auth.generateAuthHeader() })
    ).subscribe(
      (data: any) => {
        console.log(data)
